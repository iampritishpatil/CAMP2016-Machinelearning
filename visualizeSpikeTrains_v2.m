...
k_cluster   = 2;
[idx,C]         = kmeans(psth,k_cluster);

[idxspike,C]    = kmeans(allSpikes,k_cluster, 'Distance', 'hamming');

% Number of neurons in the cluster 1 
numNeurons_C1 = sum(idx==1)./numberNeur*100
numNeurons_C2 = sum(idx==2)./numberNeur*100

numNeuronsSp_C1 = sum(idxspike==1)./numberNeur*100
numNeuronsSp_C2 = sum(idx==2)./numberNeur*100
...
k_cluster   = 2;
PermVal     = 100;
clustNum    = zeros(1,PermVal);
clustNumR   = zeros(1,PermVal);

for crs = 1:PermVal
    [idx,C]         = kmeans(psth,k_cluster);
    [idxspike,C]    = kmeans(allSpikes,k_cluster, 'Distance', 'hamming');

    [idxR,C]        = kmeans(psth(:,randperm(size(psth,2))),k_cluster);
    [idxRspike,C]   = kmeans(allSpikes(:,randperm(size(allSpikes,2))),k_cluster);
    % Number of neurons within cluster 1
    clustNum(crs)   = sum(idx==1)./numberNeur*100;
    clustNumR(crs)  = sum(idxR==1)./numberNeur*100;
    clustNumspikes(crs)   = sum(idxspike==1)./numberNeur*100;
    clustNumRspikes(crs)  = sum(idxRspike==1)./numberNeur*100;
end
...
mean(clustNumspikes)
mean(clustNumRspikes)

% is the difference between randomized psth and real psth?
p_value         = ranksum(clustNum, clustNumR)
p_valuespikes   = ranksum(clustNumspikes, clustNumRspikes)

% take home message: not different than random...
